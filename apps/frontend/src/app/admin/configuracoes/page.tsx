...
export default function ConfiguracoesPage() {
  ...
  const carregarDados = useCallback(async () => {
    try {
      const [alertasData, mpData] = await Promise.all([
        api.adminAlertas.listar(),
        api.adminPagamentos.obterMercadoPago(),
      ]);
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <section className="rounded-xl border border-[var(--color-border)] bg-white p-4 shadow-sm">
            <h2 className="font-sora text-lg font-semibold text-[var(--color-text-primary)]">Mercado Pago</h2>
